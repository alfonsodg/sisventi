...
def configurat():
	...
	impuestos={}
	sql="select doc_cabecera,doc_pie,modo_impuesto,impuestos,modo_moneda,moneda,money_drawer,productos_resumen,productos_clave,empresa,tipo_servicio,wincha,servidor_smtp,from_smtp,to_smtp,fondo_caja from pos_configuracion where codigo='"+str(pos_num)+"'"
	cnt,rso=query(sql,0)
	if cnt==0:
		curses.echo()
		curses.endwin()
...
def aut():
	...
	updat()
	while 1:
		usr=win.getstr(maxy/2-2,maxx/2,15)
		if usr!="": break
	win.addstr(maxy/2-1,maxx/2-10,"Password: ")
	curses.noecho()
	updat()
	while 1:
		pwd=win.getstr(maxy/2-1,maxx/2,15)
		if pwd!="": break
	sql="select concat(nombres,' ',apellidos),nivel from usuarios_pos where codigo='"+str(usr)+"' and clave=password('"+str(pwd)+"') and nivel>0"
	cnt,rso=query(sql,0)
	if cnt>0:
		nombre=rso[0]
		nombre=nombre[:20]
...
def payment_options(apertura,cierre,archivo=''):
	...
	sql="select forma_pago,nombre,modo from formas_pago"
	...
	if archivo!='':
		...
		archivo.write(texto+'\n')
	sql="select distinct(doc.n_doc_base),doc.mntdol from docventa doc  where "+str(condicion)+" and doc.caja="+str(caja_num)+" and doc.pv="+str(pos_num)+" "+str(condicion_doc)+" and estado='B' and doc.mntdol>0"
	cuenta,resultado=query(sql,1)
	for linea in resultado:
...
#			condicion="tiempo>='"+apertura+"'"
		...
		sql="select n_doc_base,sello,total from docventa where vales like '%"+str(resultado[x][0])+"%'  and caja="+str(caja_num)+" and "+str(condicion)+" and pv="+str(pos_num)+" "+str(condicion_doc)+" and estado='B' group by n_doc_base order by n_doc_base"
...
def comprobantes(apertura,cierre,archivo):
	...
	if cierre=='0000-00-00 00:00:00':
		cierre=apertura
#		condicion="doc.tiempo between '"+apertura+"' and '"+cierre+"'"
#	else:
#		condicion="doc.tiempo>='"+apertura+"'"
	condicion="doc.fecha_vta between date('"+apertura+"') and date('"+cierre+"')"
#	if cierre!='0000-00-00 00:00:00':
#		condicion="tiempo between '"+apertura+"' and '"+cierre+"'"
#	else:
#		condicion="tiempo>='"+apertura+"'"
	sql="select doc.estado,doc.comprobante,fpa.nombre,count(distinct(doc.n_doc_base)) as cnt,sum(sub_total_bruto) as mnt from docventa doc left join documentos_comerciales fpa on fpa.documento=doc.comprobante where "+str(condicion)+" and doc.caja="+str(caja_num)+" and doc.pv="+str(pos_num)+" and doc.imod=0 group by doc.estado,doc.comprobante order by doc.estado,doc.comprobante"
	cuenta,resultado=query(sql,1)
	for linea in resultado:
		estado=linea[0]
		if estado=='B':
...
def codigo(apertura,cierre,archivo):
	if cierre=='0000-00-00 00:00:00':
		cierre=apertura
	condicion="doc.fecha_vta between date('"+apertura+"') and date('"+cierre+"')"
	sql="select doc.codigo,if(length(mae.alias)>0,left(mae.alias,29),left(concat(mae.nombre,' ',mae.descripcion),29)) as producto,doc.cantidad as cnt,doc.precio as precio,sub_codbarras from docventa doc left join maestro mae on mae.codbarras=doc.codigo where "+condicion+" and doc.estado='B' and doc.pv="+str(pos_num)+"  and doc.caja="+str(caja_num)+" and imod=0"# group by doc.codigo"
	cuenta,resultado=query(sql,1)
	sub_prod={}
	prod={}
	for linea in resultado:
...
#	else:
#		condicion="doc.tiempo between '"+apertura+"' and '"+cierre+"'"
	sql="select count(distinct(doc.n_doc_base)) as cnt,doc.cv_ing,concat(usu.nombres,' ',usu.apellidos) as usr,doc.fp,fpa.nombre,sum(sub_total_bruto) as mnt from docventa doc left join usuarios_pos usu on usu.codigo=doc.cv_ing left join formas_pago fpa on fpa.forma_pago=doc.fp where doc.estado='B' and doc.pv="+str(pos_num)+"  and doc.caja="+str(caja_num)+" and doc.imod=0 and "+str(condicion)+" group by doc.cv_ing,doc.fp"
	cuenta,resultado=query(sql)
	cod_usr=''
	total_op=0
	total_mnt=0
...
#	else:
#		condicion="doc.tiempo between '"+apertura+"' and '"+cierre+"'"
	sql="select count(distinct(doc.n_doc_base)) as cnt,date(doc.tiempo),concat(lpad(hour(doc.tiempo),2,'0'),':00') as hora,sum(sub_total_bruto) as mnt from docventa doc where "+str(condicion)+" and doc.estado='B' and doc.pv="+str(pos_num)+"  and doc.caja="+str(caja_num)+" and doc.imod=0 group by date(doc.tiempo),hour(doc.tiempo)"
	cuenta,resultado=query(sql)
	for linea in resultado:
		fecha=str(linea[1])
...
def get_correlativo(modo,documento,edit=0,panel=''):
	sql="select prefijo,correlativo,sufijo from documentos_comerciales where documento='"+str(documento)+"' and modo='"+str(modo)+"'";
	cuenta,resultado=query(sql,0)
	if cuenta>0:
		prefijo=resultado[0]
...
def set_correlativo(modo_doc,tipo_doc,dato,modo=1):
	sql="update documentos_comerciales set correlativo='"+str(dato)+"' where documento='"+str(tipo_doc)+"' and modo='"+str(modo_doc)+"'";
	if modo==1:
...
def cierrecaja():
	try:
		estado=0
		tiempo=time.strftime("%Y-%m-%d %H:%M:%S")
		sql="select id,apertura,cierre from pos_administracion where pv="+str(pos_num)+" and caja="+str(caja_num)+" and cierre='0' and apertura !='2999-12-30 23:59:59'"
		cuenta,resultado=query(sql,0)
		if cuenta==1:
			sql="update pos_administracion set cierre='"+str(tiempo)+"', user_out='"+str(codven)+"' where id='"+str(resultado[0])+"' and pv="+str(pos_num)+" and caja="+str(caja_num)+""
			exe=curs.execute(sql)
		else:
			estado+=1
		sql1="select id from pos_administracion where pv="+str(pos_num)+" and caja="+str(caja_num)+" and cierre='0' and apertura='2999-12-30 23:59:59'"
		cuenta1,resultado1=query(sql1,0)	
		if cuenta1==0:
			sql="insert into pos_administracion (apertura,pv,caja,user_ing) values ('2999-12-30 23:59:59','"+str(pos_num)+"','"+str(caja_num)+"','"+str(codven)+"')"
			exe=curs.execute(sql)
		else:
			estado+=1
		if estado==0:
...
def delivery(apertura,cierre,modo=0):
	bkg=''
	datemp=[]
	sql="select a.tiempo,c.n_doc_base,c.total,b.cliente,if(length(d.alias)>0,d.alias,concat(d.nombre,' ',d.descripcion)),c.cantidad from delivery as a,clientes as b,docventa as c,maestro as d where a.cliente=b.docnum and c.n_doc_base=a.docnum and c.codigo=d.codbarras and c.estado='B' and (a.tiempo>='"+str(apertura)+"' and a.tiempo<='"+str(cierre)+"') order by a.tiempo"
	cuenta,resultado=query(sql)
	if cuenta>0:
		lineas=[]
		for x in range(0,cuenta):
...
def selec_doc(modo):
	sql="select documento,nombre from documentos_comerciales where modo="+str(modo)+" and pv="+str(pos_num)+" and caja="+str(caja_num)+" order by documento"
	cuenta,resultado=query(sql)
	if cuenta>0:
...
def doc_man(fecha):
	docum=[]
	sql="select doc.nombre,sum(vta.sub_total_bruto) from docventa vta left join documentos_comerciales doc on doc.documento=vta.comprobante where vta.fecha_vta='"+str(fecha)+"' and doc.modo=5 and vta.estado='B' group by doc.modo,doc.documento"
	cuenta,resultado=query(sql)
	if cuenta>0:
		for linea in resultado:
...
def proc_anulacion():
	...
	updat()
	while 1:
		sql="select distinct(modo),if(modo='0','Interno','Manual') from documentos_comerciales order by modo"
		modo,modo_dscp=sql_seleccion(sql,'Modo')
		if modo=='Anular':
			return 0
		sql="select documento,nombre from documentos_comerciales where modo='"+str(modo)+"' order by documento"
		documento,documento_dscp=sql_seleccion(sql,'Documento')
		if documento=='Anular':
			return 0
		sql="select copia,detalle from documentos_comerciales where documento='"+str(documento)+"' and modo='"+str(modo)+"' and pv="+str(pos_num)+" and caja="+str(caja_num)+""
		cuenta,resultado=query(sql,0)
		if cuenta>0:
			compr_id=documento
		...
		if no_doc=='Anular':
			break
		sql="select doc.fp,fpa.nombre,doc.cv_ing,concat(usr.nombres,' ',usr.apellidos),doc.cantidad,doc.codigo,if(length(mae.alias)>0,mae.alias,concat(mae.nombre,' ',mae.descripcion)),doc.precio,doc.total,doc.tiempo,doc.cliente,dir.nombre_corto,doc.n_doc_base from docventa doc left join maestro mae on mae.codbarras=doc.codigo left join formas_pago fpa on fpa.forma_pago=doc.fp left join usuarios_pos usr on usr.codigo=doc.cv_ing left join directorio dir on dir.doc_id=doc.cliente where doc.n_doc_base='"+str(no_doc)+"' and doc.comprobante='"+str(compr_id)+"' and doc.caja="+str(caja_num)+" and doc.pv="+str(pos_num)+" and doc.tiempo>='"+str(apertura)+"'"
		cuenta,resultado=query(sql)
		if cuenta>0:
			...
			if resp=='si':
				...
				if debug_mode=='0' or debug_mode=='':
					os.remove(nomarch)
				sql="update docventa set estado='A',cv_anul='"+str(codven)+"',tiempo_null='"+str(tiempo_null)+"' where n_doc_base='"+str(no_doc)+"' and comprobante='"+str(compr_id)+"' and caja="+str(caja_num)+" and pv="+str(pos_num)+""
				exe=curs.execute(sql)
...
def gestion_caja():
	...
	updat()
	sql="select max(cierre) from pos_administracion"
	cuenta,resultado=query(sql,0)
	...
	if tiempo<=ultimo_tiempo:
		...
		win.getch()
		resp='salir'
	fecha_obt=time.strftime("%Y-%m-%d")
	sql="select id from pos_administracion where apertura like '"+str(fecha_obt)+" %'"
	cuenta,resultado=query(sql)
	if cuenta>0:
		strimpr('Existe una Apertura en el Mismo Dia',13,maxx,win)
		strimpr('No es posible Aperturar la Caja',14,maxx,win)
		updat()
		win.getch()
		resp='salir'
	while resp!='salir':
		sql="select id from pos_administracion where pv="+str(pos_num)+" and caja="+str(caja_num)+" and apertura='2999-12-30 23:59:59' order by id desc"
		cuenta,resultado=query(sql,0)
		if cuenta==1:
			...
			if resp=='si':
				sql="update pos_administracion set apertura='"+str(tiempo)+"', user_ing='"+str(codven)+"', pv="+str(pos_num)+", caja="+str(caja_num)+" where apertura='2999-12-30 23:59:59'"
				exe=curs.execute(sql)
...
def gestion_asistencia():
	...
	updat()
	while 1:
		sql="select ingreso,salida from personal where usuario='"+str(codven)+"' and pv="+str(pos_num)+" and salida='0' order by id desc"
		cuenta,resultado=query(sql,0)
		if cuenta==0:
			...
			if resp=='si':
				qsql="insert into personal (ingreso,pv,usuario) values ('"+str(tiempo)+"','"+str(pos_num)+"','"+str(codven)+"')"
				qsex=curs.execute(qsql)
			...
			if resp=='si':
				qsql="update personal set salida='"+str(tiempo)+"' where ingreso='"+str(resultado[0])+"' and usuario='"+str(codven)+"' and pv="+str(pos_num)+""
				qsex=curs.execute(qsql)
...
def vta_total(fecha):
	sql="select sum(vta.sub_total_bruto) from docventa vta where vta.fecha_vta=date('"+str(fecha)+"') and vta.estado='B' and vta.pv="+str(pos_num)+""
	cuenta,resultado=query(sql,0)
	if cuenta>0:
			...
			if ingdat=='Anular':
				...
				for z in range(0,len(lineas)):
					if lineas[z][3]=='0':
						...
						sql="update produccion_derivados pro set "+base+",user_ing='"+str(codven)+"' where pro.codbarras='"+str(lineas[z][0])+"' and pro.fecha='"+str(fecha)+"' and pro.cp_base='"+str(alm_ori)+"' and pro.cp_aux='"+str(alm_des)+"' and pro.turno='"+str(turno)+"'"
						query_trans.append(sql)
					else:
							...
							if ingdat=='insert':
								...
								for z in range(0,len(lineas)):
									if lineas[z][3]=='0':
										...
										sql="update pedidos set "+base+",user_ing='"+str(codven)+"' where fecha='"+str(fecha)+"' and n_doc_prefijo='"+str(guia_prefijo)+"' and  n_doc_base='"+str(guia)+"' and n_doc_sufijo='"+str(guia_sufijo)+"' and codbarras='"+str(lineas[z][0])+"' and modo='"+str(tipo_ped)+"' and turno='"+str(turno)+"'"
									else:
										temporal=lineas[z]
										temporal=agregar_valores(temporal,2,guia_prefijo,guia,guia_sufijo,pos_num,codven,codven,fecha,turno,tipo_ped)
