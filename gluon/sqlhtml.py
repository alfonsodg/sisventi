...
class AutocompleteWidget(object):
    ...
    def __init__(self, request, field, id_field=None, db=None,
                 ...
                 min_length=2, filterby=None, filtervalue=None, mode=None):
    ...
    def callback(self):
        if self.keyword in self.request.vars:
            ...
            if self.mode:
                if self.filterby:
                    rows = self.db((field.contains(self.request.vars[self.keyword])) & (self.filterby==self.filtervalue))\
                        .select(orderby=self.orderby,limitby=self.limitby,*self.fields)
                else:
                    rows = self.db(field.contains(self.request.vars[self.keyword]))\
                        .select(orderby=self.orderby,limitby=self.limitby,*self.fields)
            else:
                rows = self.db(field.startswith(self.request.vars[self.keyword]))\
                    .select(orderby=self.orderby,limitby=self.limitby,*self.fields)
            if rows:
    ...
    def __call__(self,field,value,**attributes):
        ...
        if self.is_reference:
            ...
            attr['_onblur']="jQuery('#%(div_id)s').delay(2000).fadeOut('fast');" % \
